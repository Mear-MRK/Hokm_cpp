  ...
  if (!ag_name.empty())
	{
		...
		cleaned_name.reserve(ag_name.size());
		for (unsigned char ch : ag_name) {
			if (std::isalnum(ch) || ch == '_' || ch == '-' || ch == ' ' || ch == '.')
				cleaned_name.push_back(static_cast<char>(ch));
		}
		if (!cleaned_name.empty()) {
			name = (cleaned_name.size() > 20) ? cleaned_name.substr(0, 20) : cleaned_name;
		}
	}
...
Suit InteractiveAgent::call_trump(const CardStack &first_5cards) {
  ...
  do {
    ...
    std::transform(inp_trump_str.begin(), inp_trump_str.end(),
                   inp_trump_str.begin(), ::toupper);
...
void InteractiveAgent::info(const std::string &info_str) { output(info_str); }