...
class HokmClient:
    ...
    def _token_path(self):
        safe_host = (self.ip_addr or "localhost").replace("/", "_").replace(":", "_")
        return os.path.expanduser(f"~/.hokm_token_{safe_host}_{self.port}")
    ...
    def _parse_tbl_pairs(self, table_str):
        # Parse "name:card;name:card;..." maintaining order
        pairs = []
        for seg in table_str.split(';'):
            ...
            name = (name or "").strip()
            card = (card or "").strip()
            pairs.append((name, card if card else "--"))
        # Ensure length 4
        while len(pairs) < 4:
            pairs.append(("", "--"))
        return pairs[:4]
    ...
    def update_table(self, table_str):
        # New format: /TBL<name>:<card>;<name>:<card>;... in absolute player order [0..3]
        pairs = self._parse_tbl_pairs(table_str)
        names = [nm for nm, _ in pairs]
        cards = [cd for _, cd in pairs]

        # Map to relative seats using assigned player_id
        u = self.player_id % 4
        a = (u + 1) % 4
        c = (u + 2) % 4
        b = (u + 3) % 4
        ...
        self.table_pan.addstr(0, lo + 2, *HokmClient.card_unc_att(card_c))
        self.table_pan.addstr(1, lo + 0, *HokmClient.card_unc_att(card_b))
        self.table_pan.addstr(1, lo + 4, *HokmClient.card_unc_att(card_a))
        self.table_pan.addstr(2, lo + 2, *HokmClient.card_unc_att(card_u))
